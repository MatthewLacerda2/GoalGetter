...
async def list_goals(
    ...
    goals_with_updates = await goal_repo.get_goals_with_latest_updates(current_user.id)
    all_goals = await goal_repo.get_by_student_id(current_user.id)
...
async def delete_goal(
    ...
    if is_active_goal:
        ...
        objective_repo = ObjectiveRepository(db)
        latest_objective = await objective_repo.get_latest_for_student_excluding_goal(current_user.id, goal_id)
    ...
    # Delete the goal and all related records cascadingly using repository
    await goal_repo.delete_cascade(goal_id)
    await db.commit()
