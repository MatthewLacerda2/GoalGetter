...
async def main(fresh: bool) -> int:
    try:
        ...
        print("claude: the tables are missing. Start the backend once (it creates the schema), then re-run.",
              file=sys.stderr)
        return 1
    ...
    if result.created:
        ...
        print(f"claude: {result.student.name} already has its history ({names}); nothing written. "
              "ARGS=--fresh rebuilds it.")
    print("  The backend drops its schema on every start: a restart wipes this history. Re-run `make claude` then.")
    return 0
