...
def moment(now: datetime, days_ago: int, hour: Optional[int], minute: int = 0) -> datetime:
...
def plan_lessons(plan: list, bank_size: int, now: datetime) -> list[PlannedLesson]:
    ...
    for i, (days_ago, hour, correct) in enumerate(plan):
        ...
        elo += delta
        planned.append(PlannedLesson(
            finished_at=moment(now, days_ago, hour),
            question_indexes=[(i * size + k) % bank_size for k in range(size)],
            correct=correct, accuracy=accuracy, elo_delta=delta, elo_after=elo,
        ))
    return planned


async def _seed_lessons(db: AsyncSession, goal: Goal, bank: list[LessonQuestion], planned: list[PlannedLesson]):
    ...
    for i, plan in enumerate(planned):
        ...
        lesson = await lessons.create(Lesson(
            goal_id=goal.id, created_at=started, question_ids=[q.id for q in served],
            finished_at=plan.finished_at, total_seconds=sum(seconds), accuracy=plan.accuracy,
            elo_delta=plan.elo_delta, elo_after=plan.elo_after,
        ))
        await answers.create_many([
            LessonAnswer(
                lesson_id=lesson.id, question_id=q.id, is_correct=k < plan.correct,
                selected_option_index=q.correct_option_index if k < plan.correct else (q.correct_option_index + 1) % 4,
                time_spent=seconds[k], created_at=started + timedelta(seconds=sum(seconds[:k + 1])),
            )
...
async def seed_goal(db: AsyncSession, student: Student, spec: dict, now: datetime) -> Goal:
    ...
    goal = await GoalRepository(db).create(Goal(
        student_id=student.id, name=spec["name"], description=spec["description"],
        rating=planned[-1].elo_after if planned else START_RATING,
        created_at=created, updated_at=planned[-1].finished_at if planned else created,
    ))
    bank = await LessonQuestionRepository(db).create_many([
        LessonQuestion(
            goal_id=goal.id, question=text, option_a=options[0], option_b=options[1],
            option_c=options[2], option_d=options[3], correct_option_index=correct,
    ...
    for prompt, replies, liked, days_ago, hour in spec["chat"]:
        await chat.create(ChatMessage(
            student_id=student.id, goal_id=goal.id, prompt=prompt, tutor_responses=replies,
            is_liked=liked, created_at=moment(now, days_ago, hour, minute=20),
        ))
    await ResourceRepository(db).create_many([
        Resource(
            goal_id=goal.id, resource_type=StudyResourceType(kind), name=name, description=description,
            language=language, link=link, created_at=created + timedelta(minutes=10),
        )
        for kind, name, description, language, link in spec["resources"]
    ])
    if spec["context"]:
        state, metacognition = spec["context"]
        await StudentContextRepository(db).create(StudentContext(
            student_id=student.id, goal_id=goal.id, state=state, metacognition=metacognition,
