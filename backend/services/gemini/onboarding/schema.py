...
class GeminiIntroductionScreens(BaseModel):
    screens: list[GeminiIntroductionScreen] = Field(
        description="3 to 5 introduction screens shown while the goal is being set up"
    )


class GeminiGoalValidation(BaseModel):
    makes_sense: bool = Field(description="Can we infer what the user said or wants?")
    is_harmless: bool = Field(
        description="The request is not immoral, ill-intended, nor harmful to anyone"
    )
