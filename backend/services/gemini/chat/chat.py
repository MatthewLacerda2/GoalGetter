...
if __name__ == "__main__":
    ...
    messages = [
        ChatMessageWithGemini(message="What are you doing?", role="model", time="10:00:13"),
        ChatMessageWithGemini(message="Hey is ma checklist right? Its for turning on the Cessna 150", role="user", time="10:00:00"),
        ...
        ChatMessageWithGemini(message="No, this checklist is incorrect.", role="model", time="10:00:00"),
        ChatMessageWithGemini(message="RPM and Fuel Flow are not settings you adjust to 'turn on' the plane. Those are readouts or power settings used after the engine is running.", role="model", time="10:00:00"),
        ChatMessageWithGemini(message="You're missing critical steps like setting the parking brake, fuel selector, mixture, and throttle properly. You also need to prime the engine.", role="model", time="10:00:00"),
        ChatMessageWithGemini(message="'Press Turn On' is too vague. You need to engage the starter.", role="model", time="10:00:00"),
        ChatMessageWithGemini(message="You need to consult the Pilot's Operating Handbook (POH) for the Cessna 150. That will give you the precise, correct, and safe startup procedure. Guessing leads to mistakes.", role="model", time="10:00:00"),
    ]
