...
class ResourceRecommendationItem(BaseModel):
    resource_type: StudyResourceType = Field(
        description="The type of the resource: pdf, webpage, or youtube"
    )
    name: str = Field(description="The name or title of the resource")
    description: str = Field(
        description="A short, descriptive summary of what this resource teaches (in 20 words or less)"
    )
...
class GeminiResourceSearchResults(BaseModel):
    resources: list[ResourceRecommendationItem] = Field(
