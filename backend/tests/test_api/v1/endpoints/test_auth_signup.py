...
async def test_signup_new_user(client, mock_google_verify):
    ...
    response = await client.post(
        "/api/v1/auth/signup", headers={"Authorization": "Bearer valid_google_token"}
    )
...
async def test_signup_existing_user(client, mock_google_verify, test_user):
    ...
    response = await client.post(
        "/api/v1/auth/signup", headers={"Authorization": "Bearer fixture_user_token"}
    )
    token_response = TokenResponse.model_validate(response.json())
    assert response.status_code == 201
...
async def test_signup_invalid_token(client, mock_google_verify):
    ...
    response = await client.post(
        "/api/v1/auth/signup", headers={"Authorization": "Bearer invalid_token"}
    )
