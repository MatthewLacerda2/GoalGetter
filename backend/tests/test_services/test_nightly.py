...
called**: a student who did no lesson costs nothing, not even a cheap call.
...
def test_a_student_who_never_finished_a_lesson_is_skipped():
    assert decide(None, THURSDAY_RUN).run is False


def test_a_student_whose_last_lesson_is_older_than_the_night_is_skipped():
...
def test_a_lesson_finished_in_the_small_hours_still_counts():
...
async def test_a_student_who_did_nothing_costs_no_gemini_call(
    ...
    await finished_lesson_factory(goal, finished_at=LAST_WEEK)
...
@pytest.mark.asyncio
async def test_a_student_who_did_a_lesson_gets_context_then_questions(
    test_db, test_user, goal_factory, finished_lesson_factory
):
    """Six nights a week the chain stops after the second step"""
    goal = await goal_factory(test_user)
    await finished_lesson_factory(goal, finished_at=WEDNESDAY_EVENING)
    await test_db.commit()
...
async def test_resources_are_searched_on_monday_and_only_then(
    ...
    goal = await goal_factory(test_user)
    await finished_lesson_factory(goal, finished_at=SUNDAY_EVENING)
    await test_db.commit()
...
async def test_a_chat_is_not_activity(
    test_db, test_user, goal_factory, exchange_factory, finished_lesson_factory
):
...
async def test_one_student_failing_does_not_stop_the_next(
    test_db, student_factory, goal_factory, finished_lesson_factory
):
...
async def test_running_it_by_hand_logs_the_decision_it_took(
    test_db, test_user, goal_factory, finished_lesson_factory, caplog
):
    ...
    assert "skipped: last lesson 2026-09-10 12:00Z is before 06:00Z" in caplog.text
